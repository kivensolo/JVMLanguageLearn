package com.nio.xul_server;


import com.nio.xul_server.utils.RecycleOutputBuffer;
import com.nio.xul_server.utils.TextUtils;
import com.nio.xul_server.utils.XulMemoryOutputStream;
...
public class XulHttpServer {
	...
	public static class XulHttpServerResponse extends XulHttpResponse {
		...
		public XulHttpServerResponse(XulHttpServerHandler handler) {
			_handler = handler;
			_outputStream = RecycleOutputBuffer.obtain(2048);
	...
	public static class XulHttpServerHandler {
		...
		/**
		 * 处理Http请求，默认不作处理，返回404
		 * 需具体功能代码实现;
		 * @param request XulHttpServerRequest
		 * @throws IOException
		 */
		...
		class HttpRequestBuilder {
			...
			public HttpRequestBuilder() {
				_readBuffer = RecycleOutputBuffer.obtain(4096);
			}
				...
				while (_parseState != 2 && (line = readLine()) != null) {
					if (_parseState == 0) {
						...
						if (headerKey.equalsIgnoreCase("content-length")) {
							_requestBodySize = TextUtils.tryParseInt(headerValue);
						} else if (headerKey.equalsIgnoreCase("host")) {
							int hostPortPos = headerValue.indexOf(":");
							if (hostPortPos > 0) {
								_request.port = TextUtils.tryParseInt(headerValue.substring(hostPortPos + 1));
