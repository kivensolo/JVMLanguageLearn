...
class ClassFile {
    ...
    var constant_pool_count:ByteArray = ByteArray(2)  // 常量池计数器(常量池实际大小为该值减1  排除了下标0)
    var cp_infos = arrayOf<ConstantPoolInfo>()            // 常量池数据区数据 size = constant_pool_count - 1


    var access_flags:ByteArray = ByteArray(2)     // 访问标志: 类还是接口, 以及是否被 Public、Abstract、Final 等修饰符修饰
    var this_class:ByteArray = ByteArray(2)       // 当前类的全限定名，值为常量池中的索引值
    var super_class:ByteArray = ByteArray(2)      // 父类的全限定名,值也是在常量池中的索引值
    var interfaces_count:ByteArray = ByteArray(2) // 类的接口计数器
